...
async function fetchMovies(page = 1) {
  ...
  if (!title) return alert("Adj meg egy filmcímet!");

  resultsDiv.innerHTML = "<p>Keresés folyamatban...</p>";
  paginationDiv.innerHTML = "";

  try {
    const url = `${omdbBaseUrl}?apikey=${omdbApiKey}&s=${encodeURIComponent(
      title
    )}&type=movie&page=${page}${year ? `&y=${year}` : ""}`;
    ...
    const moviesWithDetails = await Promise.all(
      data.Search.map(async (movie: any) => {
        const detailsRes = await fetch(
          `${omdbBaseUrl}?apikey=${omdbApiKey}&i=${movie.imdbID}&plot=full`
        );
...
function displayResults(movies: any[], page: number, totalResults: number) {
  ...
  movies.forEach((movie) => {
    ...
    card.innerHTML = `
      <div class="movie-info">
        <img src="${
          movie.Poster !== "N/A"
            ? movie.Poster
            : "https://via.placeholder.com/300x450?text=Nincs+poszter"
        }" alt="${movie.Title}" />
    ...
    const detailsBtn = card.querySelector(".details-btn") as HTMLButtonElement;
    ...
    detailsBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (detailsData.Ratings && Array.isArray(detailsData.Ratings)) {
          const rtCritic = detailsData.Ratings.find(
            (r: any) => r.Source === "Rotten Tomatoes"
          );
          if (rtCritic) rottenCritics = rtCritic.Value;
        }
...
async function fetchTmdbExtras(title: string) {
  try {
    const searchRes = await fetch(
      `${tmdbBaseUrl}/search/movie?api_key=${tmdbApiKey}&query=${encodeURIComponent(title)}`
    );
