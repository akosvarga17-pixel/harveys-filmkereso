const searchInput = document.getElementById("titleInput") as HTMLInputElement;
const yearInput = document.getElementById("yearInput") as HTMLInputElement;
const minRatingInput = document.getElementById("minRatingInput") as HTMLInputElement;
...
async function fetchMovies(page = 1) {
  ...
  const omdbUrl = `https://www.omdbapi.com/?apikey=${OMDB_API_KEY}&s=${encodeURIComponent(
    currentSearch
  )}&type=movie&page=${page}${currentYear ? `&y=${currentYear}` : ""}`;
  ...
  try {
    ...
    const withFullData = await Promise.all(
      detailedMovies.map(async (movie: any) => {
        try {
          ...
          if (tmdbMovie) {
            const tmdbDetails = await fetch(
              `https://api.themoviedb.org/3/movie/${tmdbMovie.id}?api_key=${TMDB_API_KEY}&append_to_response=credits,reviews`
            );
            const tmdbFull = await tmdbDetails.json();

            const director =
              tmdbFull.credits?.crew?.find((p: any) => p.job === "Director")?.name || "N/A";
            ...
            const criticReview =
              tmdbFull.reviews?.results?.[0]?.content ||
        ...
        movie.rotten_score =
          movie.Ratings?.find((r: any) => r.Source === "Rotten Tomatoes")?.Value || "N/A";

        return movie;
      })
    );

    const filteredMovies = withFullData.filter((m: any) => {
...
function displayMovies(movies: any[], page: number, totalResults: number) {
  resultsDiv.innerHTML = "";

  movies.slice(0, 10).forEach((movie: any) => {
    const poster =
      movie.Poster !== "N/A"
        ? movie.Poster
...
(window as any).toggleDetails = function (id: string, button: HTMLButtonElement) {
